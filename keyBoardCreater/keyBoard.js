...
class KeyBoard {
    ...
    static mouseDown() {
      document.querySelector('.keysBoard').addEventListener('mousedown', (event) => {
        if (event.target.classList.contains('button')) {
    ...
    mouseClick() {
      document.querySelector('.keysBoard').addEventListener('click', (event) => this.textareaInteraction(event));
    }
    ...
    static keyUp() {
      window.addEventListener('keyup', (event) => {
        ...
        if (constants.KEYS_EVENT_CODE.includes(event.code)) {
          test.classList.remove('key-active');
        }
      });
    }
